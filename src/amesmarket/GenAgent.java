...
/**
 * GenCo - a wholesale power producer.
 *
 * <p>
 ...
 * <tr>
 * <th>ID</th>	<th>atBus</th>	<th>FCost</th>	<th>A</th> <th>B</th> <th>CapMin</th> <th>CapMax</th> <th>initMoney</th>
 * </tr>
 * <tr>
 * <td>1</td>	<td>1</td> <td>5</td> <td>15</td> <td>0.10</td> <td>0</td> <td>100</td> <td>1000</td>
 * </tr>
 * </table>
 */

public class GenAgent implements Drawable {
    ...
    // JReLM component
    //private SimpleStatelessLearner<double[]> learner;
    ...
    ArrayList<double[]> newActionDomain;

    private String genID;

    /**
     * Value for PowerGeneratedT0 in SCUC input file.
     * This is the default t0 power only. Once the simulation
     * starts running this will be ignored.
     */
     ...
     * generator. Certain calculations (e.g. gen capacity) need to
     * exclude the canary generators.
     */
    ...
    public GenAgent(double[] genData, VREParameters learningParams,
                    ArrayList<double[]> actionDomain, double ss, double dCap, int random, int iStart, int iLength, double dCheck, boolean bCheck,
        ...
        //learner = new SimpleStatelessLearner<double[]>(learningParams, newActionDomain);
        ...
        genID = "GenCo" + id; //defualt id until/unless another is set
    ...
    public ArrayList<double[]> checkActionDomain(ArrayList<double[]> actionList) {
        ArrayList<double[]> newActionList=new ArrayList<double[]>();
        for(int i=0; i<actionList.size(); i++) {
            double [] action= actionList.get(i);
    ...
    public double getMaxPotentialProfit( ) {
        ...
        for(int i=0; i<newActionDomain.size(); i++) {
            double [] action= newActionDomain.get(i);
...
//        double[] actionTriplet = learner.chooseActionRaw();
...
//    }
    ...
    public double[] submitTrueSupplyOffer() {
        ...
        if(maxPrice<=priceCap) {
            ...
            trueOffer[3]=trueSupplyOffer[3];
//                      This need to be discussed with Dr. Tesfatsion       
//                    (priceCap-trueSupplyOffer[0])/(2*trueSupplyOffer[1]);
...
//        SimpleAction<double[]> lastAction = learner.getPolicy().getLastAction();
//        choiceID = lastAction.getID();
//        double[] act = lastAction.getAct();
//        REPolicy policy = (REPolicy)learner.getPolicy();
//        /*
...
//        return action;
    ...
    /**
     * Whether or not the generator is a 'canary' that
     * is used to indicate the load was too much for the
     * standard generators. This is a simple way to deal
     * with the lack of reserve problem.
     */
    ...
    public ArrayList<double[][]> getDayAheadLMPByDay() {
...
//    }
    ...
    /**
     * Check to see if there are dispatch values for hours when the
     * gen co was not committed. Writes error messages directly to sys out.
     * @param day
     */
    ...
    /**
     *
     * Put the 'actual', that is realtime computed dispatch
     * for the GenCo.
    ...
    /**
     * Put the commitment for day d. This stores the commitment
     * by day d, but the values are generated by the SCUC/DAM
     * on day d-1. In otherwords, if we are running the DAM on
     * day d, add commitment for d+1.
     * @param d
     * @param commitment
     */
    ...
    * if negative, the number of hours prior to (and including) t=0 that the unit has been off.
     ...
     * </blockquote>
     * @param day
     * @return The on/off is whether or not the unit was on/off on the last hour of day-1,
     * or, 1, if the generator does not have a record for day d-1.
     *
     */
    ...
    /**
     * Set the no load cost
     * pre: d >= 0.0 (not enfoced)
     * @param d
     */
